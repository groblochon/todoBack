<template>
  <div>
    <h2 id="page-heading" data-cy="TodoHeading">
      <span v-text="t$('backApp.todo.home.title')" id="todo-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('backApp.todo.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'TodoCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-todo">
            ...
            <span v-text="t$('backApp.todo.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && todos && todos.length === 0">
      <span v-text="t$('backApp.todo.home.notFound')"></span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="t$('backApp.todo.name')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'name'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('content')">
              <span v-text="t$('backApp.todo.content')"></span>
            ...
            <th scope="row" v-on:click="changeOrder('done')">
              <span v-text="t$('backApp.todo.done')"></span>
            ...
            <th scope="row" v-on:click="changeOrder('login.id')">
              <span v-text="t$('backApp.todo.login')"></span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="backApp.todo.delete.question" data-cy="todoDeleteDialogHeading" v-text="t$('entity.delete.title')"></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-todo-heading" v-text="t$('backApp.todo.delete.question', { id: removeId })"></p>
      </div>
